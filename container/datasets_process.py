...
from dynamic_factory import DynamicFactory
...
def dataset(srcpaths: RdeInputDirPaths, resource_paths: RdeOutputResourcePath) -> None:
    ...
    if config.get("xrd").get("main_image_setting"):
        repeated_meta_items: dict = {}
        repeated_meta_items['main_image_setting'] = [config["xrd"]['main_image_setting'] for _ in range(region_num)]
        module.meta_parser.repeated_meta_info.update(repeated_meta_items)
    module.meta_parser.save_meta(resource_paths.meta.joinpath("metadata.json"), Meta(metadata_def))

    # Overwrite invoice's specimen.
    module.invoice_writer.overwrite_invoice_if_needed(
        resource_paths.invoice.joinpath("invoice.json"), resource_paths,
    )
